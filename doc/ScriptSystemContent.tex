...
There are three registered global functions. The first one prints the string in the parameter to the log (\verb/void Log(string& message)/) so it is useful for debugging scripts. The second and the last are the \verb/int32 GetState()/ and \verb/void SetAndSleep(int32, uint64)/ which can be called only from a special functions (see section \ref{script-component} for details). For a complete reference of registered classes, global functions etc. see ScriptRegister.pdf.
