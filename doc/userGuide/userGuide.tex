...
Throughout the user guide, the basic game editing techniques will be presented on a sample pinball game, which
will be created from scratch. This guide concentrates only on real basics, so the game will be rather simple
but complex enough to demonstrate all necessary use cases.
...
On that page, you can find the brief description of the project, location of project resources, such as source
code repository and installation packages. To download the latest installation package, navigate your browser
to the \href{http://sourceforge.net/projects/ocerus/files/}{Ocerus download page} and download the latest
version.
...
guide from the editor, the resources are likely to be in the \href{run:./archive/}{archive/} directory.
Otherwise, you need to download the archive package from the project website. It should be somewhere near the
user guide.
...
discussed later. Let's start with creating a new project.
...
your own consideration, but it's recommended to stick with the default structure that is created for new
projects.
...
In the \textbf{New Scene} dialog, select the scenes sub-directory and type \texttt{gameplay} as the scene
...
There are five views: \textbf{Entity}, \textbf{Hierarchy}, 
...
design the pinball machine. Nevertheless, duplicating entities is not recommended in this case. 
...
our scene that defines two flipper buttons.
\lstinputlisting[language=xml,
	basicstyle=\scriptsize,
	stringstyle=\ttfamily,
	showstringspaces=false,
...
in the \textbf{Entity} view. Then drag the \texttt{gameplay.layout} file to the \textbf{Layout} property.
Finally check the \textbf{Enabled} and \textbf{Visible} checkboxes.
