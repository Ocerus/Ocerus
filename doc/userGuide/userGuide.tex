...
Welcome to the Ocerus user guide. With Ocerus you can create modern 2D games in an easy and
...
it should help you set up and begin creating wonderful games in no time. 
...
On that page, you can find project resources, such as the source code repository and installation packages. To
...
window appears (see figure~\ref{fig:ocerus_initial}). Near the upper edge of the window there is the main menu. You can
...
Project} dialog. Type \texttt{Pinball} into the name field and browse to the location where your project will
...
another project. Behind the scene, projects are ordinary directories in your file system with resource files
representing your game content, such as images, models, scripts, etc. The structure of the project directory is on
...
In the \textbf{New Scene} dialog click on the folder icon to create a subfolder. Name it \texttt{scenes} and then
enter this directory after it's created. Then type in \texttt{gameplay} as the scene
...
{\itshape
Scenes are your virtual playgrounds where the game takes place. Generally speaking they consist of an
indefinite plane with your game elements, such as the player's avatar or enemies, on it. You can think of a scene
as a game level. 
}

After a scene is opened, viewports and views are displayed (see figure~\ref{fig:new_scene_created}). Viewports
are framed areas that provide a view of the current scene. There are two viewports in the Ocerus editor. The
...
Viewports are surrounded by views that provide information related to the current scene or the current entity.
There are five views: \textbf{Entity}, \textbf{Hierarchy}, \textbf{Layers}, \textbf{Prototypes} and
\textbf{Resources}. All of these will be discussed later. Now let's move on to create the protagonist of our
...
To create an entity, select the \textbf{Edit $\to$ New Entity} command from the main menu and type the desired name of the entity into the edit box that has appeared. In our sample project there will be a ball that will bounce over the area, so we name the entity
...
As soon as an entity is selected, the \textbf{Entity} view shows information related to it (see
...
\textbf{General} section in the \textbf{Entity} view and these properties can be edited the same way as the name property mentioned above.
...
Every file and directory that is placed into the project directory is visible in the
\textbf{Resources} view. To put files with your textures into the project, simply copy
them somewhere into the project directory. Remember that you cannot achieve this from
Ocerus, so use your favorite file manager to do this.

When you add a file into the project directory, Ocerus automatically detects the file
and creates an appropriate resource representing it. You can tell the type of the resource
by the icon next to the resource name in the \textbf{Resource} view.
...
\section{Controlling the editor camera}
...
This is, however, very uncomfortable. Ocerus provides editing tools to do this easily. There are
four editing tools in Ocerus: the \textbf{move} tool, the \textbf{rotate} tool, the \textbf{rotate-y} tool and
the \textbf{scale} tool. They are accessible from the toolbar located to the right from the main menu (see
figure~\ref{fig:editing_tools}). You can switch between the editing tools by clicking the corresponding icon.
...
\capstart
\begin{center}
  \includegraphics[scale=0.8]{PolygonCollider}
 \end{center}
 \vspace{-20pt}
...
\texttt{Ball} entity (select \textbf{Edit $\to$ New Component $\to$ PolygonCollider}). The new component now
...
editor viewport. If you have used your own texture, you may need to modify these points to make the polygon aligned
with the texture.

You may wonder how we have figured out these numbers. Polygon vertices are defined in game coordinates and the point
with both values set to zero is the center of the entity. One unit in game coodrinates corresponds to 50 pixels of
your texture. Let's consider the following example: we want to find a collision polygon that would outline a square
image with dimensions $100 \times 100$ pixels. The polygon will have points [-1, -1], [1, -1], [1, 1] and [-1, 1].

Our ball entity has a collision polygon now, which means that if physics is applied to that entity, it will
collide with another entities accordingly to their collision polygons. So to really see the
...
components to the entity: \textbf{StaticBody} or \textbf{DynamicBody}.
...
entities with the \textbf{DynamicBody} component are used for objects that are affected by the gravitation force
and other forces. Our ball entity is an example of an entity with the \textbf{DynamicBody} component.
...
Instead, all objects are dragged straight down. It means that as soon as we start the game our ball will infinitely fall down. To stop it we need to create some barriers in our world.

\textbf{Task 1.} Create a new entity called \texttt{Platform}, add \textbf{Sprite} component and use the
\texttt{textures$\backslash$steel.jpg} file from the \hyperref[sec:archive]{archive} as the texture. Add
the \textbf{PolygonCollider} component and set the polygon points to [5.12, -5.12], [5.12, 5.12], [-5.12, 5.12],
[-5.12, -5.12]. Add the \textbf{StaticBody} component and finally use the edit tools to shape the square into a
...
This is because the action is stopped. While editing you can arrange the initial state of your scene. Then, when you want to test what you have done, you can animate the scene by starting the action. Action is controlled
...
scene to the initial state. After the action is started, you can still use the edit tools and edit entity
properties. However, changes that you make will be discarded as soon as you stop the action.
...
triple the number of the vertices and the movement will be smoother. Note that Ocerus does not provide
colliders for round shapes.
...
is not exactly short and you probably don't want to imagine how long it would take to create all those new
entities the same way. For purposes of reusing existing entities there are few tricks in Ocerus. The first
...
the \texttt{Prototypes.xml} file and can be accessed from any scene in the project. When you need to create a
...
prototype to the viewport couple of times to have enough entities to build the pinball machine.
...
\capstart
\begin{center}
  \includegraphics[scale=0.8]{SharedProperties}
 \end{center}
 \vspace{-20pt}
...
across several scenes in your project. You only have to change the prototype and Ocerus will do the rest.

\begin{wrapfigure}{r}{0.35\textwidth}
\vspace{-20pt}
\capstart
\begin{center}
  \includegraphics[scale=0.8]{LockedProperties}
 \end{center}
 \vspace{-20pt}
...
Select the \texttt{Platform} entity from the \textbf{Hierarchy} view and take a look at the \textbf{Entity}
view again (see figure~\ref{fig:locked_properties}). All properties that are shared have a lock icon to the right from them. That means they can only be edited in the prototype.
...
to build all barriers. Also create the \texttt{Flipper} prototype that would be similar to the \texttt{Platform}
prototype, except that it will use the \texttt{textures$\backslash$yellow.jpg} texture from the
\hyperref[sec:archive]{archive} and it will contain the \textbf{DynamicBody} component instead of the \textbf{StaticBody}
...
unlink it by clicking the remove button on the right of the \textbf{Prototype} property in
...
As the number of entities in your scene grows, it becomes harder not to get lost. Fortunately, Ocerus
provides means for hierarchical organization through the \textbf{Hierarchy} view. Unlike the file system, each and every entity can become a parent of other entities, so there are no folders in this
...
elements), you need to use the \textbf{reparent up} action of the context menu couple of times. Just right click on the entity
and select the \textbf{reparent up} action and the entity will move one level upwards in the hierarchy.

Entities can also be ordered among theirs siblings. Use the \textbf{move up} and \textbf{move down} commands
...
\capstart
\begin{center}
  \includegraphics[scale=0.8]{Hierarchy}
 \end{center}
 \vspace{-20pt}
...
the \textbf{Transform} component by clicking the red cross to the right of the component title in the
\textbf{Entity} view. Then move all platform entities to the Platforms entity. Do the same with flippers (see
...
your pinball machine. So far we didn't have to take the order of sprites into account, but now we need
to make sure that background stays behind the pinball machine. This can be achieved by layers.  

\section{Using layers}

If you look at the \textbf{Layers} view, you can see the \texttt{Initial Layer} there. Every scene must have
at least one layer and this one is automatically created. You can click on the plus sign to the left of the
...
\capstart
 \begin{center}
  \includegraphics[scale=0.8]{LayersView}
 \end{center}
 \vspace{-20pt}
 \caption{Layers view}
...
the active layer, double click on another layer and it becomes active. You can also toggle the layer
visibility by clicking the eye icon to the right of the layer.

The visual appearance of our scene is done, as is physics. Now it's time to write the
underlying scripts to implement the game logic.

\section{Scripting}
The Ocerus engine allows you to implement the game behavior using scripts. Although writing scripts is
indeed programming, it differs from the core Ocerus programming in many ways. First, the Ocerus scripting environment
was designed for game designers, rather than senior C++ programmers. It does not overwhelm you with
unnecessary complexity of the "guts" of the engine, but it rather provides only the relevant things that are
...
The \textbf{Script} component allows you to implement a custom behavior using the scripting language. As soon as
...
\texttt{scripts$\backslash$flipper.as} file from the \hyperref[sec:archive]{archive}).
...
Back in Ocerus, select the \texttt{Flipper} prototype and click the green plus button next to the
\textbf{ScriptModules} property. Drag the \texttt{flipper.as} resource from the \textbf{Resources} view to the
\textbf{NULL} field in the modules list and click the blue diskette button.

\begin{figure}[ht]
 \begin{center}
  \includegraphics[scale=0.70]{ScriptComponent}
 \end{center}
 \caption{Script component of Flipper prototype}
 \label{fig:script_component}
\end{figure}

If everything went well, your script gets loaded and a couple of new properties appear in the \textbf{Script}
component part of the \textbf{Entity} view (see figure~\ref{fig:script_component}). These properties are defined
in the script and they allow you to control the scripted behavior. In our script, there are two properties:
\textbf{LeftSided} and \textbf{Pivot}. The \textbf{LeftSided} property determines which flipper is the left
one and which is the other one and the \textbf{Pivot} property defines the location of the pivot point.
...
on the toolbar and try to control your flippers with the \textbf{Left} and \textbf{Right} keys.
...
\section{GUI}
The last topic that is covered in this user guide is GUI creation. Ocerus uses the CEGUI library that
...
as they are just XML files, however CEGUI provides visual editors for some of them. You can download them at the CEGUI website.
...
The \textbf{Font} files allow you to define fonts for your GUI, whether they are TrueType fonts or bitmap fonts.
Again, Ocerus provide a reasonable default font so neither \textbf{Font} files are discussed in detail
here. See the \href{http://www.cegui.org.uk/api_reference/xml_font.html}{CEGUI manual} to get more info about the
\textbf{Font} files.

The \textbf{Imageset} files allow to define your custom images that can be used in GUI. CEGUI allows you to put
multiple images into a single big image file and then specify which parts of the big image correspond to particular
images. This map is defined in the \textbf{Imageset} files. In our \texttt{Pinball} project, we will define faces
of two buttons that allow the player to trigger flippers by clicking them. Look at the \textbf{Imageset} file for our
buttons.
...
their name, size and position within the image.
...
\lstinputlisting[language=c,
	...
	breaklines=true,
	caption=gameplay.as,
	label={gameplay.as}]{archive/gui/gameplay.as}
You may have noticed that the GUI windows above contain the \textbf{Event} element. What it does exactly is described in the sections below but basically when an event is generated it attempts to execute the given function in the callback script attached to the layout.	

To add the GUI to the project, copy the \texttt{gui} directory from the \hyperref[sec:archive]{archive} to the
project directory. Then switch back to Ocerus and look at the \textbf{Resources} view. Ocerus tries to
...
files must to be changed to the \textbf{CEGUI} resource. Right click the \texttt{buttons.png} file in the
...
The \verb/<GUILayout>/ element is the root element in the XML file. It must contain a single \verb/<Window>/ element representing the root GUI element. The \verb/<Window>/ element must have the \verb/Type/ attribute which specifies the type of the window to be created. This may refer to a concrete window type, an alias, or a falagard mapped type. It can also have the \verb/Name/ attribute specifying a unique name of the window. This element may contain \verb/<Property>/ elements with the \verb/Name/ and \verb/Value/ attributes. They are used to set properties of the window. It may also contain \verb/<Event>/ elements with the \verb/Name/ and \verb/Function/ attributes which are then used to create bindings between the window and script functions (see section \ref{sec:events}). Also, it can contain another \verb/<Window>/ elements as its child windows. For supported window types, properties and events see the \href{http://www.cegui.org.uk/api_reference/fal_man.html}{CEGUI documentation}.
...
The encoding of text files must be ASCII or UTF8. It is possible to end lines in the Windows (\verb/\r\n/) or the UNIX (\verb/\n/) style.
...
\section{Deployment}
After you are finished with your game, it's very easy to deploy it. Open the \textbf{File $\to$ Deploy Project} menu and select the target platform. Then select the target directory in the following dialog. After you click \textbf{OK}, your game gets deployed into the selected directory.
...
In this part you can see the description of all features the editor can perform using menus and other mouse actions. First of all take a look at picture \ref{fig:editor_description}. There you can see parts of the editor which will be described in the sections below. What is not visible in the picture is a developer console. It can be opened and closed by pressing the ` key.
...
Here you can see all entities in the game structured as a tree. If you click an item the entity gets selected and will appear both in the Entity view and in the Editor viewport as well. If you double-click the item the Editor viewport will pan to the entity (if it has the Transform component, so to speak). You can drag the entities around in the Hierarchy view to change the structure of the tree. Right-clicking an item will bring up the following menu:
		\begin{tabular}{|p{0.23\hsize}|p{0.70\hsize}|}
			\hline
