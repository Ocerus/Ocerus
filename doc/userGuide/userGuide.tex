...
In the \textbf{New Scene} dialog click on the folder icon to create a subfolder. Name it \texttt{scenes} and enter it after it's created. Then type in \texttt{gameplay} as the scene
...
To create an entity, select the \textbf{Edit $\to$ New Entity} command from the main menu and type in the entity
...
Every file and directory that is placed in the project directory hierarchy is visible in the
\textbf{Resources} view. To put files with your textures into the project, simply copy them somewhere into the
project directory. When you add a file into the project directory, Ocerus automatically detects the file and creates a suitable resource representing it. The type of the resource is shown by an icon next to
