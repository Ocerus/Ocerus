...
{
	BS_ASSERT(1==0); // FIXME: not correct yet: use is_valid() as a temporary workaround 
	tree_node *tmp=pos.node;
...
{
	BS_ASSERT(position.node!=head);
	BS_ASSERT(position.node);
...
{
	BS_ASSERT(head->next_sibling==feet);
...
//    {
//    BS_ASSERT(1==0); // this routine is not finished yet.
//    while(from!=to) {
//       if(fun(*subfrom, *from)) {
//          
//          }
...
public:
	...
	{
		mRoot.mFirstChild = 0;
		mRoot.mNextSibling = 0;
	}

	class SiblingIterator
	{
	public:
		SiblingIterator(const Node* node, const Node* parent): mNode(node), mParentNode(parent) {}
		SiblingIterator(const SiblingIterator& rhs): mNode(rhs.mNode), mParentNode(rhs.mParentNode) {}
		SiblingIterator& operator=(const SiblingIterator& rhs)
		{
			mNode = rhs.mNode;
			mParentNode = rhs.mParentNode;
			return *this; 
		}
		SiblingIterator& operator++(void)
		{
			BS_ASSERT(mNode);
			mNode = mNode->mNextSibling;
		}
		T operator*(void) { BS_ASSERT(mNode); return mNode->mData; }
		bool operator==(const SiblingIterator& rhs) { return mNode == rhs.mNode; }
		bool HashNext(void) { BS_ASSERT(mNode); return mNode->mNextSibling != 0; }
	private:
		Node* mNode;
		Node* mParentNode;
		SiblingIterator(void): mNode(0), mParentNode(0) {}
		Node* GetNode(void) { BS_ASSERT(mNode); return mNode; }
		Node* GetParentNode(void) { return mParentNode; }
	};

	template<typename Iter>
	Iter GetRoot(void) { return Iter(&mRoot, 0); }

	template<typename Iter>
	Iter End(void) { return Iter(); }

	template<typename Iter>
	Iter GetFirstChild(const Iter parent) { return Iter(parent.GetNode()->mFirstChild, parent.GetNode()); }

	template<typename Iter>
	Iter FindChild(const Iter parent, const T key)
	{
		Node* parentNode = parent.GetNode();
		if (!parentNode)
			return End();
		for (Node* n=parentNode->mFirstChild; n; n=n->mNextSibling)
			if (n->mData == key)
				return Iter(n, parentNode);
		return End();
	}
	...
	Iter AddChild(const Iter parent, const T key)
	{
		...
		{
			parentNode->mFirstChild = newNode;
			return Iter(newNode, parentNode);
		}
		Node** last=&parentNode->mFirstChild;
		for (Node* i=parentNode->mFirstChild; i; last=&i->mNextSibling, i=i->mNextSibling)
			if (data > i->mData)
			{
				*last = newNode;
				newNode->mNextSibling = i->mNextSibling;
				return Iter(newNode, parentNode);
			}
		*last = newNode;
		return Iter(newNode, parentNode);
	...
	template<typename Iter>
	inline void Delete(const Iter toDel) { Delete(toDel.GetNode(), toDel.GetParentNode()); }
...
private:
	...
	template<typename V>
	SearchTree(const SearchTree<V>& rhs) {}

	template<typename V>
	SearchTree& operator=(const SearchTree<V>& rhs) { return *this; }

	template<typename Iter>
	void Delete(const Node* node, const Node* parentNode)
	{
		if (!node)
			return;
		while (node->mFirstChild)
			Delete(node->mFirstChild, node);

		if (!parentNode)
			return;

		for (Node* n=parentNode->mFirstChild, Node** last=&parentNode->mFirstChild; n; n=n->mNextSibling)
			if (n == node)
			{
