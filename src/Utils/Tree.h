...
{
	OC_ASSERT(1==0); // FIXME: not correct yet: use is_valid() as a temporary workaround
	tree_node *tmp=pos.node;
...
{
	OC_ASSERT(position.node!=head);
	OC_ASSERT(position.node);
...
{
	OC_ASSERT(head->next_sibling==feet);
...
//    {
//    OC_ASSERT(1==0); // this routine is not finished yet.
//    while(from!=to) {
//       if(fun(*subfrom, *from)) {
//
//          }
...
public:
	...
	public:
		...
		T operator*(void) { OC_ASSERT(mNode); return mNode->mData; }
		bool operator==(const SiblingIterator& rhs) { return mNode == rhs.mNode; }
		bool HashNext(void) { OC_ASSERT(mNode); return mNode->mNextSibling != 0; }
	private:
		Node* mNode;
		Node* mParentNode;
		SiblingIterator(void): mNode(0), mParentNode(0) {}
		Node* GetNode(void) { OC_ASSERT(mNode); return mNode; }
