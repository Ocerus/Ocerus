...
void RegisterArrayObject(asIScriptEngine *engine)
{
	...
	r = engine->RegisterObjectBehaviour("_builtin_array_<T>", asBEHAVE_FACTORY, "_builtin_array_<T>@ f(int&in, uint, const T &in)", asFUNCTIONPR(ArrayObjectFactoryDefVal, (asIObjectType*, asUINT, void *), asCArrayObject*), asCALL_CDECL); asASSERT( r >= 0 );
	...
	r = engine->RegisterObjectBehaviour("_builtin_array_<T>", asBEHAVE_FACTORY, "_builtin_array_<T>@ f(int&in, uint, const T &in)", asFUNCTION(ArrayObjectFactoryDefVal_Generic), asCALL_GENERIC); asASSERT( r >= 0 );
