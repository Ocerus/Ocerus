...
class CDebugHelp
{
	...
	private:
		// current dump type
		static uintx m_DumpType;

		// dump type conversion table (DumpType -> MINIDUMP_TYPE)
		static const uintx m_DumpTypeConverter[];
...
class CSymbolResolver
{
	...
	public:
		...
		void ResolveSymbol(uintx p_Address, tstring& p_Function, tstring& p_File, uintx& p_Line);

		// resolve symbols for a specified address and module
		void ResolveSymbol(uintx p_Address, ModuleHandle p_hModule, const tchar* p_ModuleName, tstring& p_Function, tstring& p_File, uintx& p_Line);
